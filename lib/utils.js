...
exports.matchBangumi = async function matchBangumi(input, options) {
  ...
  for (const item of options.items) {
    ...
    if (options.matchMyanimelistId === true) {
      ...
      fileToAdd = [];
      for (const localFile of existingBangumi) {
        const titleMatches = [];
        for (const existingBangumi of localFile.result) {
          ...
          for (const title of titles) {
            const matchResult = options.matchTitle(existingTitles, title);
            const isTitleMatch = matchResult && typeof matchResult.then === 'function'
              ? await matchResult
              : matchResult;
            if (isTitleMatch) {
          ...
          if (matchedTitle) {
            if (options.matchBegin) {
              // check begin date
              const isBeginValid = options.matchBegin(existingBangumi.begin, item);
              const beginValid = isBeginValid && typeof isBeginValid.then === 'function'
                ? await isBeginValid
                : isBeginValid;
              if (!beginValid) {
                matchedTitle = null;
          ...
          if (matchedTitle) {
            titleMatches.push(matchedTitle);
          }
        }
